...
class NotchSkyLightWindow: NSPanel {
    ...
    /// The notch is normally a passive overlay that never steals keyboard focus.
    /// The text-input composer (Control-double-tap) is the one exception: while it
    /// is on screen the window must be allowed to become key so its `TextField`
    /// can receive keystrokes. Because the panel is `.nonactivatingPanel`, becoming
    /// key does NOT activate the (LSUIElement) app — focus is granted for input only
    /// while whatever app was frontmost stays frontmost. Toggled by the AppDelegate
    /// in response to `.perchShowTextInput` / `.perchTextInputDidDismiss`.
