...
enum DS {
    ...
    enum Colors {
        ...
        // ── Notch Panel (HeyPerch design) ───────────────────────────
        // Tokens for the notch-anchored panel that replaced the menu bar
        // dropdown. The panel is pure black so it reads as an extension of
        // the physical notch, with slightly raised near-black surfaces.

        /// Notch panel background — pure black, flush with the notch.
        static let notchPanelBackground = Color(hex: "#000000")

        /// Selected tab pill / keycap chip fill inside the notch panel.
        static let notchChipFill = Color(hex: "#1E2022")

        /// Raised container rows (integrations strip, swatch buttons).
        static let notchSurface = Color(hex: "#131517")

        /// Slightly lighter button fill (the "Dock" button, send button).
        static let notchButtonFill = Color(hex: "#2A2C2E")

        /// Subtle border on notch surfaces.
        static let notchBorder = Color.white.opacity(0.08)

        // ── Glint Keycaps ────────────────────────────────────────────
        // The premium black keyboard keycaps used to *display* (not edit)
        // Perch's hotkeys in the Settings page. Near-black face with a
        // bright top-edge glint and a hairline outline so the cap reads as
        // a real, physical key catching light.

        /// Top of the keycap's vertical face gradient (slightly lifted black).
        static let notchKeyCapFaceTop = Color(hex: "#262829")
        /// Bottom of the keycap's vertical face gradient (deep black).
        static let notchKeyCapFaceBottom = Color(hex: "#0C0D0E")
        /// The bright sheen drawn along the keycap's top edge — the "glint".
        static let notchKeyCapGlint = Color.white.opacity(0.55)
        /// Hairline outline around the keycap.
        static let notchKeyCapBorder = Color.white.opacity(0.14)

        // ── Cursor Color Presets ─────────────────────────────────────
        // The four selectable Perch cursor colors shown in the notch
        // panel's "Cursor color" section. Blue is the classic default
        // (same value as overlayCursorBlue).

        static let cursorRed = Color(hex: "#FF4338")
        static let cursorBlue = Color(hex: "#3380FF")
        static let cursorYellow = Color(hex: "#F5B921")
        static let cursorGreen = Color(hex: "#34C759")

        // ── Agent Card Tints ─────────────────────────────────────────
        // Muted colored backgrounds for completed agent run cards in the
        // notch panel's Agents tab (navy / green / maroon rotation).

        static let agentCardNavy = Color(hex: "#1C2A4E")
        static let agentCardGreen = Color(hex: "#163322")
        static let agentCardMaroon = Color(hex: "#3B2023")
    ...
    /// Geometry for the teardrop notch silhouette (`NotchShape`) shared by the
    /// always-on closed pill and the open HUD panel.
    enum Notch {
        /// Radius of the top "shoulders" where the surface meets the screen
        /// edge — kept small so the top reads as flush with the physical notch.
        static let shapeTopCornerRadius: CGFloat = 6
        /// Radius of the rounded bottom corners of the surface.
        static let shapeBottomCornerRadius: CGFloat = 14
        /// How far the closed pill drops below the physical notch. A small
        /// visible "chin" (notch's software-notch look) so the notch UI
        /// is actually present at rest; set to 0 to camouflage it completely.
        static let closedPillDropHeight: CGFloat = 10
    }
...
// MARK: - Voice Aura Visual Style

/// Palette for the voice-active notch animation (the pulsing orb on the right
/// and the tracing line around the notch outline). A deliberate, documented
/// exception to the "one accent color" rule (see DESIGN.md): instead of a flat
/// fill it uses a gentle *blue-family* spread — light cyan → system blue → deep
/// blue — so it reads as "an ocean of color" while staying tied to the system
/// blue (`cursorBlue` / `accent`). All cool tones, never a full rainbow.
enum VoiceAuraVisualStyle {
    /// Conic ring colors for the orb (loops back to the first color so the
    /// rotation has no visible seam).
    static let sphereRing: [Color] = [
        Color(hex: "#AEE3FF"), Color(hex: "#5BB0FF"), Color(hex: "#3380FF"),
        Color(hex: "#2E6BFF"), Color(hex: "#4B7CFF"), Color(hex: "#8FD2FF"),
        Color(hex: "#AEE3FF")
    ]
    /// The orb's specular highlight tint (the bright core that gets pushed to
    /// the rim during the "speaking" repulsion).
    static let coreTint = Color(hex: "#C8E6FF")
    /// Tracing-line gradient, top (where it meets the screen) to bottom-center.
    static let lineTop = Color(hex: "#60A5FA")
    static let lineMid = Color(hex: "#3380FF")
    static let lineBottom = Color(hex: "#2E6BFF")
    /// Shared soft glow color for both the orb and the line.
    static let glow = Color(hex: "#3C82FF")
}

/// A resolved set of voice-aura colors. Defaults to the blue family, but can be
/// derived from an arbitrary accent color (e.g. the now-playing album art's
/// dominant color) so the orb + tracing line match whatever's playing.
struct VoiceAuraPalette {
    let sphereRing: [Color]
    let coreTint: Color
    let lineTop: Color
    let lineMid: Color
    let lineBottom: Color
    let glow: Color

    /// The default blue family (used when no music is playing).
    static let blue = VoiceAuraPalette(
        sphereRing: VoiceAuraVisualStyle.sphereRing,
        coreTint: VoiceAuraVisualStyle.coreTint,
        lineTop: VoiceAuraVisualStyle.lineTop,
        lineMid: VoiceAuraVisualStyle.lineMid,
        lineBottom: VoiceAuraVisualStyle.lineBottom,
        glow: VoiceAuraVisualStyle.glow
    )

    /// Build a same-family aura from a base accent color: a light tint at the top
    /// of the line / the orb's rim highlight, the accent in the middle, and a
    /// deeper shade at the bottom. Keeps the lit tones bright enough to read even
    /// when the album color is dark.
    static func from(accent base: Color) -> VoiceAuraPalette {
        let light = base.blendedWithWhite(fraction: 0.55)
        let lightish = base.blendedWithWhite(fraction: 0.22)
        let deep = base.blendedWithBlack(fraction: 0.22)
        return VoiceAuraPalette(
            sphereRing: [light, lightish, base, deep, base, lightish, light],
            coreTint: base.blendedWithWhite(fraction: 0.7),
            lineTop: light,
            lineMid: base,
            lineBottom: deep,
            glow: base
