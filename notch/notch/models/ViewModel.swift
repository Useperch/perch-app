...
class ViewModel: NSObject, ObservableObject {
    ...
    let animationLibrary: Animations = .init()
    ...
    /// The width of the black center strip that must exactly cover the physical
    /// notch, so the content flanking it (the "ears": album art, agent icon, orb)
    /// sits at the SAME margin from the notch edge no matter which closed-notch
    /// live-activity is showing or how large the notch is. Every closed-state
    /// surface (voice / music / agent / face) uses this single value — previously
    /// each subtracted a different ad-hoc inset (0, 6, 20), which drifted the ears
    /// relative to the real notch and looked worse on smaller notches.
    var physicalNotchCenterWidth: CGFloat {
