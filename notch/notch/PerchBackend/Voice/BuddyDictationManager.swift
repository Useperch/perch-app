...
final class BuddyDictationManager: NSObject, ObservableObject {
    ...
    private func startPushToTalk(
        ...
        // Ears turned off in the menu → don't open the microphone at all (and don't
        // prompt for permission). Both entry points — the ⌃⌥ keyboard shortcut and
        // the mic button — funnel through here, so this one guard covers voice input.
        guard PerchCapabilityToggles.isEarsEnabledNow() else {
            print("🎙️ BuddyDictationManager: Ears disabled — push-to-talk suppressed")
            return
        }
        ...
        let activeTranscriptionSession = try await transcriptionProvider.startStreamingSession(
            ...
            onTranscriptUpdate: { [weak self] transcriptText in
                Task { @MainActor in
                    ...
                    // Push the live partial transcript to the draft callback so
                    // the UI can stream what the user is saying in real time,
                    // not just on finalize.
