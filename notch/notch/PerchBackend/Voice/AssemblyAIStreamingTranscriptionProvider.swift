...
final class AssemblyAIStreamingTranscriptionProvider: BuddyTranscriptionProvider {
    ...
    /// Read from the `TranscribeTokenURL` Info.plist key so it can point at a
    /// different worker than the chat/TTS routes (e.g. during local dev).
    private static let tokenProxyURL = AppBundleConfiguration.stringValue(forKey: "TranscribeTokenURL")
        ?? "https://your-worker-name.your-subdomain.workers.dev/transcribe-token"
