...
struct PerchOverlaySizePreferenceKey: PreferenceKey {
...
struct LiveTranscriptionSizePreferenceKey: PreferenceKey {
    static var defaultValue: CGSize = .zero
    static func reduce(value: inout CGSize, nextValue: () -> CGSize) {
        value = nextValue()
    }
...
struct BlueCursorView: View {
    ...
    /// Size of the live-transcription / completion bubble, tracked so it can be
    /// positioned beside the cursor without overlapping it.
    @State private var liveTranscriptionBubbleSize: CGSize = .zero
    ...
    private let fullWelcomeMessage = "hey! i'm perch"

    /// The user-selected cursor color from the notch panel — drives the
    /// triangle, waveform, spinner, and bubble accents in this overlay.
    private var cursorColor: Color {
        companionManager.cursorColor.color
    }

    /// The buddy triangle is full size while following the cursor, but shrinks
    /// to the compact size while parked beside the notch (docked + idle) so it
    /// doesn't loom over the small notch area — matching the agent indicators.
    private var cursorTriangleSize: CGFloat {
        let isDockedAndResting = companionManager.isCursorDocked && companionManager.voiceState == .idle
        return isDockedAndResting ? PerchCursorMetrics.compactTriangleSize : PerchCursorMetrics.triangleSize
    }
    ...
    var body: some View {
        ZStack {
                ...
                Text(welcomeText)
                    ...
                    .overlay(
                        GeometryReader { geo in
                            Color.clear
                                .preference(key: PerchOverlaySizePreferenceKey.self, value: geo.size)
                        }
                    )
                    ...
                    .onPreferenceChange(PerchOverlaySizePreferenceKey.self) { newSize in
                        bubbleSize = newSize
                    }
            }
            ...
            Triangle()
                .fill(cursorColor)
                .frame(width: cursorTriangleSize, height: cursorTriangleSize)
                .rotationEffect(.degrees(triangleRotationDegrees))
                .shadow(color: cursorColor, radius: 8 + (buddyFlightScale - 1.0) * 20, x: 0, y: 0)
                .scaleEffect(buddyFlightScale)
                .opacity(buddyIsVisibleOnThisScreen && companionManager.funnyCursorEmoji == nil && (companionManager.voiceState == .idle || companionManager.voiceState == .responding) ? cursorOpacity : 0)
            ...
            // Funny emoji costume — replaces the triangle when the user has
            // tapped the docked cursor to morph it. Stays in the tree and
            // cross-fades with the triangle via opacity (same pattern as the
            // waveform/spinner). Rendered upright (no rotation) so the emoji
            // reads right-side up wherever the cursor goes.
            Text(companionManager.funnyCursorEmoji ?? "")
                .font(.system(size: 18))
                .scaleEffect(buddyFlightScale)
                .opacity(buddyIsVisibleOnThisScreen && companionManager.funnyCursorEmoji != nil && (companionManager.voiceState == .idle || companionManager.voiceState == .responding) ? cursorOpacity : 0)
                .position(cursorPosition)
                ...
                .animation(.easeIn(duration: 0.25), value: companionManager.voiceState)
                .animation(.easeInOut(duration: 0.2), value: companionManager.funnyCursorEmoji)
            ...
            // Blue spinner — shown while the AI is processing (transcription + Claude + waiting
            // for TTS). Background browser subagent work no longer spins the main
            // cursor — that work is signalled by the top-right agent swarm (a
            // spinning triangle per working agent) instead.
            BlueCursorSpinnerView(cursorColor: cursorColor)
                .opacity(buddyIsVisibleOnThisScreen && companionManager.voiceState == .processing ? cursorOpacity : 0)
                .position(cursorPosition)
                .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
                .animation(.easeIn(duration: 0.15), value: companionManager.voiceState)

            // Live transcription bubble — streams what the user is saying in real
            // time while they hold push-to-talk, the same way Perch's spoken
            // responses are typed out. Sits just to the side of the waveform.
            if isCursorOnThisScreen
                && companionManager.voiceState == .listening
                && !companionManager.liveTranscriptionText.isEmpty {
                Text(companionManager.liveTranscriptionText)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 10, style: .continuous)
                                    .stroke(cursorColor.opacity(0.45), lineWidth: 1)
                            )
                            .shadow(color: cursorColor.opacity(0.35), radius: 10, x: 0, y: 0)
                    )
                    .overlay(
                        GeometryReader { geo in
                            Color.clear
                                .preference(key: LiveTranscriptionSizePreferenceKey.self, value: geo.size)
                        }
                    )
                    .position(
                        x: cursorPosition.x + 18 + (liveTranscriptionBubbleSize.width / 2),
                        y: cursorPosition.y + 30 + (liveTranscriptionBubbleSize.height / 2)
                    )
                    .animation(.spring(response: 0.2, dampingFraction: 0.6, blendDuration: 0), value: cursorPosition)
                    .onPreferenceChange(LiveTranscriptionSizePreferenceKey.self) { newSize in
                        liveTranscriptionBubbleSize = newSize
                    }
            }

            // Background task completion bubble — when a background browser task
            // finishes, Perch surfaces the result here while speaking it aloud.
            if isCursorOnThisScreen && !companionManager.backgroundTaskCompletionText.isEmpty {
                Text(companionManager.backgroundTaskCompletionText)
                    .font(.system(size: 12, weight: .medium))
                    .foregroundColor(.white)
                    .lineSpacing(2)
                    .multilineTextAlignment(.leading)
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 10, style: .continuous)
                            ...
                            .overlay(
                                RoundedRectangle(cornerRadius: 10, style: .continuous)
                                    .stroke(cursorColor.opacity(0.5), lineWidth: 1)
                            )
                            .shadow(color: cursorColor.opacity(0.4), radius: 12, x: 0, y: 0)
                    )
                    .overlay(
                        GeometryReader { geo in
                            Color.clear
            ...
            // The top-right background-agent triangle swarm was retired — a running
            // agent's presence now lives in the notch itself (the Agents page: a
            // closed live-activity chip and the open "now running" hero).
        ...
        .onAppear {
            ...
            updateCursorPositionProbeIfNotchScreen()
    ...
    private func startTrackingCursor() {
        timer = Timer.scheduledTimer(withTimeInterval: 0.016, repeats: true) { _ in
            ...
            // Detect hover over an agent-swarm triangle and surface it so the
            // preview panel can expand. Done by polling here (not via SwiftUI
            // .onHover) so it works for the non-key floating preview panel.
            self.updateAgentIndicatorHover(mouseLocation: mouseLocation)
            ...
            // Docked mode — while idle, the buddy parks beside the notch
            // (its "house") instead of following the mouse. Listening,
            // processing, or responding releases it back to the cursor so
            // interactions still happen where the user is working.
            if self.companionManager.isCursorDocked && self.companionManager.voiceState == .idle {
                if let dockPosition = self.notchDockPosition() {
                    self.isCursorOnThisScreen = true
                    self.cursorPosition = dockPosition
                } else {
                    // Not the notch screen — only the notch screen shows the
                    // docked buddy.
                    self.isCursorOnThisScreen = false
                }
                return
            }
            ...
            self.updateCursorPositionProbeIfNotchScreen()
        }
    }

    /// Records the buddy's current position into the shared probe so the agent
    /// swarm can read it on demand for mitosis spawn/merge. Only the notch
    /// screen's overlay updates it (the swarm lives on the notch screen, in its
    /// coordinate space). Cheap plain assignment — does not trigger re-renders.
    private func updateCursorPositionProbeIfNotchScreen() {
        guard isNotchScreen else { return }
        companionManager.cursorPositionProbe.currentBuddyPosition = cursorPosition
    }

    /// Sets each run's `isAgentIndicatorHovered` based on whether the mouse is over
    /// THAT run's triangle slot, so only the hovered agent's preview panel expands.
    /// Only assigns on change (the timer runs 60fps; re-publishing an unchanged value
    /// would re-render the panel every frame).
    private func updateAgentIndicatorHover(mouseLocation: NSPoint) {
        guard isNotchScreen else { return }
        let activeIndicators = companionManager.backgroundAgentIndicatorStore.activeIndicators
        let mousePointInOverlay = convertScreenPointToSwiftUICoordinates(mouseLocation)
        let layout = AgentSwarmLayout(screenFrame: screenFrame)

        for indicator in activeIndicators {
            let slotCenter = layout.slotPosition(forSlotIndex: indicator.slotIndex)
            let distance = hypot(mousePointInOverlay.x - slotCenter.x, mousePointInOverlay.y - slotCenter.y)
            let isHoveringThisTriangle = distance <= AgentSwarmLayout.hoverHitRadius
            // The indicator id IS the subagent id, so it resolves the matching run.
            guard let hoveredRun = companionManager.browserSubagentManager.run(for: indicator.id) else { continue }
            if hoveredRun.isAgentIndicatorHovered != isHoveringThisTriangle {
                hoveredRun.isAgentIndicatorHovered = isHoveringThisTriangle
            }
        }
    }

    /// True when this overlay covers the screen that has the hardware notch —
    /// the only screen that hosts the docked buddy and the agent swarm.
    private var isNotchScreen: Bool {
        let notchScreen = NSScreen.screens.first(where: { $0.safeAreaInsets.top > 0 }) ?? NSScreen.main
        return notchScreen?.frame == screenFrame
    }

    /// Where the docked buddy parks, in this overlay's SwiftUI coordinates:
    /// just to the right of the notch, vertically centered in the menu bar
    /// strip. Returns nil when this overlay isn't on the notch screen.
    private func notchDockPosition() -> CGPoint? {
        let notchScreen = NSScreen.screens.first(where: { $0.safeAreaInsets.top > 0 }) ?? NSScreen.main
        guard let notchScreen, notchScreen.frame == screenFrame else { return nil }

        var notchHalfWidth: CGFloat = 100
        if let leftArea = notchScreen.auxiliaryTopLeftArea, let rightArea = notchScreen.auxiliaryTopRightArea {
            notchHalfWidth = (screenFrame.width - leftArea.width - rightArea.width) / 2
        }

        let menuBarStripHeight = max(notchScreen.safeAreaInsets.top, NSStatusBar.system.thickness)
        return CGPoint(
            x: screenFrame.width / 2 + notchHalfWidth + 18,
            y: menuBarStripHeight / 2
        )
    }
...
class OverlayWindowManager {
    ...
    /// When true, all overlay creation is suppressed — `showOverlay` becomes a
    /// no-op. Used during the staged notch backend bring-up to run the
    /// voice pipeline (Slice 2) with zero cursor-overlay windows, since those
    /// full-screen panels are the known conflict with the notch. Cleared in
    /// Slice 3 once overlay/notch coexistence is verified. This is the single
    /// chokepoint — every overlay call site funnels through `showOverlay`.
    ...
    func showOverlay(onScreens screens: [NSScreen], companionManager: CompanionManager) {
        ...
        for screen in screens {
            ...
            // The buddy cursor, in a transparent, click-through overlay.
